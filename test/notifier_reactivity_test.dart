...
void main() {
  testWidgets('every notifier rebuilds its watchers with updated state', (tester) async {
    final c = makeContainer(connections: sampleConnections());
    var builds = 0;
    String? rendered;

    final probe = Consumer(builder: (ctx, ref, _) {
      builds++;
      ...
      rendered = 'nav=$nav toasts=$toasts font=$font sel=$sel sessions=$sessions transfers=$transfers';
      return Text(rendered!, textDirection: TextDirection.ltr);
    });

    await tester.pumpWidget(UncontrolledProviderScope(container: c, child: probe));
    await tester.pump();

    // Helper: run a mutation, pump, and assert the watcher rebuilt.
    Future<void> step(String label, void Function() mutate, String expectFragment) async {
      final before = builds;
      mutate();
      await tester.pump();
      expect(builds, greaterThan(before), reason: 'no rebuild after $label');
      expect(rendered, contains(expectFragment), reason: 'stale state after $label');
    }

    await step('nav.go', () => c.read(navProvider.notifier).go(AppScreen.queue), 'nav=AppScreen.queue');
    await step('toasts.push',
        () => c.read(toastsProvider.notifier).push('t', 's', ToastKind.info), 'toasts=1');
    await step('settings.setUiFontSize',
        () => c.read(settingsProvider.notifier).setUiFontSize(14), 'font=14.0');
    await step('connections.create',
        () => c.read(connectionsProvider.notifier).create(), 'sel=New connection');
    await step(
        'sessions.openSession',
        () => c.read(sessionsProvider.notifier)
            .openSession(c.read(connectionsProvider).connections.firstWhere((x) => x.isS3)),
        'sessions=2');
    await step(
        'transfers.debugSetTransfers',
        () => c.read(transfersProvider.notifier).debugSetTransfers([
              Transfer(name: 'f', route: 'r', direction: TransferDirection.upload, sizeBytes: 1, session: 's'),
            ]),
        'transfers=1');
  ...
  test('container.listen receives an emission from every notifier', () async {
    ...
    expect(fired, greaterThanOrEqualTo(1), reason: 'historyProvider did not emit');
  });
}
