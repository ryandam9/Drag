...
void main() {
  group('SftpBackend.dispose', () {
    test('a disposed backend fails fast instead of reusing a dead session', () async {
      ...
      await expectLater(
          b.write('/f', const Stream<Uint8List>.empty(), 0), throwsStateError);
      await expectLater(b.makeDir('/d'), throwsStateError);
      await expectLater(b.delete('/f', isDir: false), throwsStateError);
    });

    test('sizeOf on a disposed backend reports "unknown" rather than throwing', () async {
