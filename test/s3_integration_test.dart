...
void main() {
  test(
    'real S3 round-trip: upload → list → download',
    () async {
      ...
      expect(
        s3.isReady,
        isTrue,
        reason: 'S3 connection should be ready with creds',
      );
  ...
  test(
    'real S3 → S3 cross-bucket copy (two accounts)',
    () async {
      final svc = TransferService();
      final local = LocalBackend();
      ...
      expect(
        (await b.list('copied/')).map((e) => e.name),
        contains('report.bin'),
      );
    ...
    skip: (_endpoint.isEmpty || _bucket2.isEmpty)
        ? 'set S3_ENDPOINT + S3_BUCKET2 to run'
        : false,
  );

  test(
    'real S3 file operations: makeDir, write, rename, delete',
    () async {
      ...
      expect(
        (await s3.list('')).where((e) => e.isDir).map((e) => e.name),
        contains('ops'),
      );
