...
void main() {
  ...
  Transfer t(String name, TransferStatus s) => Transfer(
      name: name, route: 'Local → s3://bucket/very/long/destination/path/here',
      direction: TransferDirection.upload, sizeBytes: 248 * 1024 * 1024, session: 's', status: s);
  ...
  testWidgets('Transfer Queue fits at the minimum size', (tester) async {
    ...
    c.read(transfersProvider.notifier).debugSetTransfers([
      ...
      t('a-rather-long-file-name-that-could-overflow.bin', TransferStatus.error),
  ...
  testWidgets('Transfer Queue fits at a larger text scale', (tester) async {
    final c = makeContainer();
    c.read(transfersProvider.notifier).debugSetTransfers([t('x.bin', TransferStatus.active)]);
