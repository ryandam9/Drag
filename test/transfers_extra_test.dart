...
void main() {
  ...
  group('disposal', () {
    test(
      'disposing mid-transfer aborts in-flight runs without throwing',
      () async {
        ...
        final queued = state.transfers.firstWhere(
  ...
  group('pause / resume through the queue', () {
    test(
      'pause keeps the staging partial and resume finishes from it',
      () async {
        ...
        expect(
          partial.existsSync(),
          isTrue,
          reason: 'pause must keep the partial',
        );
  ...
  group('folder-walk batching', () {
    test(
      'a big tree is published in slices, not one emission per file',
      () async {
        final c = makeContainer();
        ...
        final count = await q.enqueueTree(
          src,
          dst,
          const FileItem(name: 'folder', isDir: true),
        );
        expect(count, 120);
