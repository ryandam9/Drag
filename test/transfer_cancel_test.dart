...
void main() {
  test(
    'an aborted transfer leaves a pre-existing destination untouched',
    () async {
  ...
  group('pause vs cancel (staged local destination)', () {
    ...
    test(
      'pause mid-stream keeps the partial; resume continues from it',
      () async {
    ...
    test(
      'discardPartial removes the staging file a pause left behind',
      () async {
        final dstPath = '${dir.path}/dst.bin';
        final t = download();
        final partial = File('$dstPath.drag-partial-${t.id}')
