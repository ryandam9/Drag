...
void main() {
  ...
  group('LocalBackend ranged reads', () {
    test(
      'openReadRange streams from an offset and reports the remaining size',
      () async {
  ...
  group('TransferService resume', () {
    ...
    test('a retry resumes a partial download instead of restarting', () async {
      ...
      expect(
        await File(dstPath).readAsBytes(),
        payload,
      ); // partial + tail == full
    ...
    test(
      'a first attempt overwrites a stale file (no accidental resume)',
      () async {
        final payload = List<int>.generate(1000, (i) => i % 256);
        final src = File('${dir.path}/src.bin')..writeAsBytesSync(payload);
        final dstPath = '${dir.path}/dst.bin';
    ...
    test(
      'a successful local transfer leaves no .drag-partial behind',
      () async {
        ...
        expect(
          dir.listSync().where((e) => e.path.contains('.drag-partial')),
          isEmpty,
        );
      },
    );
    ...
    test(
      'an aborted local overwrite preserves the existing file and cleans the temp',
      () async {
        final src = File('${dir.path}/src.bin')
          ..writeAsBytesSync(List<int>.generate(1000, (i) => i % 256));
        final dstPath = '${dir.path}/keep.bin';
        final original = List<int>.filled(64, 0xAB);
        File(
          dstPath,
        ).writeAsBytesSync(original); // the file the user is overwriting
        ...
        await TransferService().run(
          ...
          control: TransferControl()
            ..abort(), // pause/cancel before any bytes flow
        );
    ...
    test(
      'checksum verification disables resume (hashes the whole file)',
      () async {
        final payload = List<int>.generate(1000, (i) => i % 256);
        final src = File('${dir.path}/src.bin')..writeAsBytesSync(payload);
        final dstPath = '${dir.path}/dst.bin';
        ...
        File(
          '$dstPath.drag-partial-${t.id}',
        ).writeAsBytesSync(payload.sublist(0, 400));
        await TransferService().run(
          t: t,
          src: LocalBackend(),
          srcPath: src.path,
