...
Connection _c(String name, {String tag = '', String host = '', String bucket = '', String region = '', String user = ''}) =>
    Connection(name: name, tag: tag, host: host, bucket: bucket, region: region, username: user,
        protocol: bucket.isNotEmpty || region.isNotEmpty ? Protocol.s3 : Protocol.sftp);
...
void main() {
  group('connectionMatches', () {
    ...
    test('matches name, tag, host and username (case-insensitive)', () {
      final c = _c('Prod SFTP', tag: 'Production', host: 'box.example.com', user: 'deploy');
  ...
  group('filterConnections', () {
    test('keeps matching connections in order', () {
      ...
      expect(filterConnections(list, 'a').map((c) => c.name), ['alpha', 'beta', 'gamma']);
  ...
  group('groupConnections', () {
    test('tagged groups sort alphabetically, untagged last, order preserved', () {
      ...
      expect(groups.map((g) => g.label), ['Production', 'Staging', kUngroupedLabel]);
    ...
    test('whitespace-only tags count as untagged', () {
      final groups = groupConnections([_c('a', tag: '  '), _c('b', tag: 'Prod')]);
