...
class AboutScreen extends ConsumerWidget {
        ...
        _card(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              ...
              style: FsType.sans(size: 13, color: FsColors.text2, height: 1.6),
            ),
          ]),
        ),
      ...
      child: Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ClipRRect(
            ...
            child: Image.asset('assets/icons/drag_512.png',
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                Text('Drag',
                    style: FsType.sans(size: 30, weight: FontWeight.w800, color: FsColors.text1)),
                const SizedBox(width: 12),
                _versionChip(),
              ]),
              const SizedBox(height: 6),
              Text('A cross-platform file-transfer client for Local, Amazon S3 & SFTP.',
                  style: FsType.sans(size: 14, color: FsColors.text2, height: 1.4)),
              const SizedBox(height: 14),
              Wrap(spacing: 8, runSpacing: 8, children: [
                for (final p in const ['macOS', 'Linux', 'Windows']) _platformChip(p),
              ]),
            ]),
          ),
        ]),
  ...
  Widget _versionChip() => Container(
        ...
        child: Text('v$_version',
            style: FsType.sans(size: 12, weight: FontWeight.w700, color: FsColors.accentHi)),
      );
  ...
  Widget _platformChip(String label) => Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(label, style: FsType.sans(size: 11, weight: FontWeight.w600, color: FsColors.text2)),
        ]),
      );
    ...
    return _card(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(
            child: Text(title,
                style: FsType.sans(size: 14, weight: FontWeight.w700, color: FsColors.text1)),
          ),
        ]),
        const SizedBox(height: 10),
        Text(body, style: FsType.sans(size: 12.5, color: FsColors.text2, height: 1.55)),
      ]),
    );
  }
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Text(name,
              style: FsType.mono(size: 12, weight: FontWeight.w700, color: FsColors.accentHi)),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: Text(desc, style: FsType.sans(size: 12.5, color: FsColors.text2, height: 1.5)),
        ),
      ]),
    );
  }
  ...
  // ── Shared bits ──────────────────────────────────────────────────────────
  Widget _heading(IconData icon, String title) => Row(children: [
