...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Widget _content(
    ...
    final appearance = _sectionBlock(
      ...
      _appearance(settings, notifier, toast),
    );
    final browser = _sectionBlock(
      'Browser',
      Icons.folder_open_outlined,
      _browser(settings, notifier),
    );
    final transfers = _sectionBlock(
      'Transfers',
      Icons.swap_vert_outlined,
      _transfers(settings, notifier),
    );
    final security = _sectionBlock(
  ...
  List<Widget> _appearance(
    AppSettings settings,
    SettingsNotifier notifier,
    void Function(String, String, ToastKind) toast,
  ) {
    return [
      _card(
        children: [
          ...
          Text(
            'Light or dark surfaces, or follow your operating system.',
      ...
      _card(
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Bird-inspired palettes — the accent recolors the whole UI.',
            style: FsType.sans(size: 11, color: FsColors.text3, height: 1.4),
          ),
          const SizedBox(height: 16),
          Wrap(
            ...
            children: [
              for (final t in kBirdThemes)
                _themeSwatch(
                  t,
                  selected: settings.themeName == t.name,
                  onTap: () {
                    notifier.setTheme(t);
        ...
        children: [
          ...
          FormField2(
            ...
            _select(
              ...
              (v) =>
                  notifier.setUiFontSize(double.parse(v.replaceAll('px', ''))),
            ),
          ),
          const SizedBox(height: 16),
          FormField2(
      ...
      child: MouseRegion(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(FsColors.rField),
            border: Border.all(
              color: selected ? FsColors.accent : FsColors.border,
              width: selected ? 2 : 1,
            ),
          ),
          child: Row(
      ...
      child: MouseRegion(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(FsColors.rField),
            border: Border.all(
              color: selected ? t.accentHi : FsColors.border,
              width: selected ? 2 : 1,
            ),
          ...
          child: Row(
            children: [
              ...
              if (selected)
                Icon(Icons.check_circle, size: 16, color: t.accentHi),
            ],
          ),
        ),
      ),
      ...
      _card(
        children: [
          _check(
            ...
            notifier.setShowHiddenFiles,
          ),
          _check(
            'Show file permissions column',
            settings.showPermsColumn,
            notifier.setShowPermsColumn,
          ),
        ],
      ),
    ];
  ...
  String _speedLabel(int kbps) => _speedLimits.entries
      .firstWhere(
        (e) => e.value == kbps,
        orElse: () => const MapEntry('Unlimited', 0),
      )
      .key;
  ...
  List<Widget> _transfers(AppSettings settings, SettingsNotifier notifier) {
    return [
      _card(
        children: [
          _check(
            'Show transfer log on startup',
            settings.showLogOnStartup,
            notifier.setShowLogOnStartup,
          ),
          _check(
            'Confirm before overwriting files',
            settings.confirmOverwrite,
            notifier.setConfirmOverwrite,
          ),
          _check(
            'Notify when a transfer finishes (window unfocused)',
        ...
        children: [
          FormField2(
            ...
            _select(
              ...
              (v) {
                final level = _verifyLabels.entries
      ...
      _card(
        children: [
          FormField2(
            ...
            _select(
              _conflictLabels[settings.conflictPolicy] ??
                  _conflictLabels['ask']!,
              _conflictLabels.values.toList(),
              (v) {
                final policy = _conflictLabels.entries
                    .firstWhere((e) => e.value == v)
                    .key;
        ...
        children: [
          Row(
            children: [
              ...
              if (store != null &&
                  store.status == KnownHostsStoreStatus.memoryOnly) ...[
                Container(
          ...
          if (store == null)
            Text(
              'Host-key storage is unavailable.',
            ...
            FutureBuilder<List<KnownHost>>(
              ...
              builder: (ctx, snap) {
                ...
                if (hosts.isEmpty) {
                  return Text(
                    'No trusted hosts yet — connect to an SFTP server to add one.',
                    style: FsType.sans(size: 12, color: FsColors.text3),
                  );
                }
                return Column(
                  children: [for (final h in hosts) _hostRow(store, h)],
                );
              },
            ),
        ],
    ...
    return _card(
      children: [
        Row(
          children: [
            Icon(
              healthy ? Icons.lock_outline : Icons.warning_amber_rounded,
              size: 18,
              color: color,
            ),
            const SizedBox(width: 8),
