...
class _TransferQueueScreenState extends ConsumerState<TransferQueueScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = q.isEmpty
        ? state.transfers
        : state.transfers.where((t) => _matches(t, q)).toList();
    return Container(
        ...
        children: [
          Row(
            children: [
              ...
              if (s.errorCount > 0)
                TbButton('↺ Retry failed', onTap: n.retryAllFailed),
                  ...
                  child: Row(
                    children: [
                      StatusBadge(
                        ...
                        bg: FsColors.badgeLocalBg,
                        fg: FsColors.accentHi,
                      ),
                      const SizedBox(width: 6),
                      StatusBadge(
                        '⊡ Queued  ${s.queuedCount}',
                        bg: FsColors.badgeQueuedBg,
                        fg: FsColors.badgeQueuedFg,
                      ),
                      const SizedBox(width: 6),
                      StatusBadge(
                        '✓ Done  ${s.doneCount}',
  ...
  Widget _table(
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Drag a file between panes in the Browser to start one.',
    ...
    if (visible.isEmpty) {
      return Container(
        alignment: Alignment.center,
        child: Text(
          'No transfers match "${_query.trim()}"',
  ...
  ({Color bg, Color fg, String label}) _badge(TransferStatus s) => switch (s) {
    ...
    TransferStatus.paused => (
      bg: FsColors.badgePausedBg,
      fg: FsColors.badgePausedFg,
      label: '⏸ Paused',
    ),
  };
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: Text(
                          ...
                          style: FsType.sans(size: 12, color: dirColor),
                        ),
                      ),
                      // File + route.
                      Expanded(
                      ...
                      Expanded(
                        ...
                        child: t.status == TransferStatus.error
                            ...
                            : (t.status == TransferStatus.queued &&
                                  t.attempts > 0)
                            ? Text(
                                'Retrying… (attempt ${t.attempts + 1}/${TransfersNotifier.maxAttempts})',
                                style: FsType.sans(
                                  size: 10,
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          if (t.sourcePath.isNotEmpty)
            FsButton(
              'Copy source',
              onTap: () => copy('Source path', t.sourcePath),
            ),
          if (t.destPath.isNotEmpty)
            FsButton(
              'Copy destination',
              onTap: () => copy('Destination path', t.destPath),
            ),
  ...
  Widget _detailsBody(Transfer t) {
    ...
    final rows = <(String, String)>[
      (
        'Direction',
        t.direction == TransferDirection.upload ? 'Upload' : 'Download',
      ),
  ...
  Widget _statsBarBody(TransfersState s, TransfersNotifier n) {
    ...
    final transferred = transfers.fold<double>(
      0,
      (sum, t) => sum + t.sizeBytes * t.progress,
    );
